/** Blog Orchestrator — app.js (구버전 index.html 호환, API 로그인 없음) */
let pollTimer = null;
...
async function loadStatus() {
  ...
  if (runBtn) runBtn.disabled = data.isRunning;
...
async function loadLogs() {
  ...
  if (logs) logs.textContent = data.lines.join("\n") || "로그가 없습니다.";
}
...
async function runPipeline() {
  ...
  try {
    ...
    msgEl.textContent = result.title
      ? `완료: ${result.title}`
      : result.message;
...
async function uploadSession(platform, file) {
  ...
  try {
    ...
    if (msgEl) msgEl.textContent = `${platform} 세션 업로드 완료`;
    await loadStatus();
  } catch (err) {
    if (msgEl) {
      msgEl.textContent = err.message;
...
function startPolling() {
  ...
  pollTimer = setInterval(async () => {
    try {
      ...
      if (data.isRunning) await loadLogs();
    } catch {
      /* ignore */
    }
  }, 5000);
